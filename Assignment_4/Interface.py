...
def RangeQuery(ratingsTableName, ratingMinValue, ratingMaxValue, openconnection):
    ...
    if (ratingMinValue == 0):
        cur.execute(
            "select PartitionNum from RangeRatingsMetadata where (%f >= MinRating and %f <= MaxRating) or (%f >= MinRating and %f <= MaxRating) or (MinRating>= %f and MaxRating<= %f);"
    ...
    result = []
    for row in rows:
        cur.execute("select * from rangeratingspart%s where rating >= %f and rating <= %f;" % (
        row[0], ratingMinValue, ratingMaxValue))
    ...
    for i in range(0, num_partition):
        cur.execute("select * from roundrobinratingspart%d where rating >= %f and rating <= %f;" % (
        i, ratingMinValue, ratingMaxValue))
        records = cur.fetchall()
        for record in records:
